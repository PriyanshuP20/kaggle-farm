...
RANCHER_DIV = _p("rancher_div", 4)     # ranchers = ceil(work / this)
...
MELON_CAP = int(_p("melon_cap", 10)); MELON_DIV = _p("melon_div", 3)
...
def _herd_plan(drain):
    """Fixed plan, sliced to HERD_TARGET (tunable). Beyond 24 the pattern
    cycles — extra slots are goose-heavy on purpose."""
    full = ["GOOSE", "GOOSE", "GOOSE", "SHEEP", "COW", "GOOSE",
            "SHEEP", "GOOSE", "COW", "GOOSE", "SHEEP", "COW",
            "GOOSE", "GOOSE", "SHEEP", "GOOSE", "COW", "GOOSE",
            "GOOSE", "SHEEP", "GOOSE", "COW", "GOOSE", "GOOSE"]
    while len(full) < HERD_TARGET:
        full.extend(full[:HERD_TARGET - len(full)])
    return full[:HERD_TARGET]
...
def _agent_impl(obs, config):
    ...
    for crop, target in _crop_targets(day, last_day, n_units, money, drain, opp).items():
        ...
        if money - spend - cost < CASH_FLOOR:
            deficit = max(0, int((money - spend - CASH_FLOOR) // CROPS[crop]["seed"]))
            cost = deficit * CROPS[crop]["seed"]
        if deficit > 0:
    ...
    if herd_total > 0 and wheat_in_shed < wheat_reserve and len(market) < 10:
        ...
        if money - spend - need * FEED_EST > CASH_FLOOR:
    ...
    if (money - spend > BUY_GATE_MONEY and day < last_day + HERD_BUY_LAST_DAY
        ...
        for a in remaining:
            ...
            if have_home and money - spend > ANIMALS[a]["cost"] + BUY_BUF:
