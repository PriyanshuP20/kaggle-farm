...
BUY_GATE_MONEY = _p("buy_gate", 1631)  # cash needed before animal buys
...
SB_CAP = int(_p("sb_cap", 12)); SB_DIV = _p("sb_div", 4)
TOM_CAP = int(_p("tom_cap", 4)); TOM_DIV = _p("tom_div", 4)
...
def _herd_plan(drain, opp_prod):
    """Top-replay profile: cow-anchored herd (~60% cows, ~25% geese, ~15%
    sheep). The insight that matters: EVERY animal prints ~$80/day of
    collectible fertilizer regardless of product price — so herd composition
    is a logistics question, not a market one. Milk/eggs are gravy; the
    fertilizer is the meal. (Demand-adaptive herds were tried 3x and all
    regressed — churning the plan mid-ramp costs more than a saturated milk
    curve.)"""
    ...
    if money > STRUCT_GATE and n_units >= BUY_UNITS and day < last_day - 6:
        ...
        for k, want in want_struct.items():
            for _ in range(max(0, want - have_struct.get(k, 0))):
                need_ops.append(k)
        if need_ops:
            # Structures go on the tiles NEAREST the shed — every feed,
            # place, and fertilizer trip is a shed round-trip, so distance
            # here is paid daily. Crops never visit the shed (produce
            # auto-drops into it at end of day), so they take the far tiles.
