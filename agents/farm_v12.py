...
def _herd_plan(drain, opp_prod):
    ...
    n_cows = -(-HERD_TARGET * 3 // 5)
    n_geese = HERD_TARGET // 4
    n_sheep = HERD_TARGET - n_cows - n_geese
    out, pools = [], [("COW", n_cows), ("GOOSE", n_geese), ("SHEEP", n_sheep)]
    order = ["COW", "COW", "GOOSE", "COW", "SHEEP"]
    while sum(p[1] for p in pools) > 0:
        for a in order:
            for i, (pa, n) in enumerate(pools):
                if pa == a and n > 0:
                    out.append(a)
                    pools[i] = (pa, n - 1)
                    break
