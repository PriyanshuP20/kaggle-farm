...
def _agent_impl(obs, config):
    ...
    for ui, pos in enumerate(units):
        ...
        # --- crop duty ---
        # Sticky ownership: each unit owns the tiles it has worked today and
        # services them first — a watering circuit forms around the unit
        # instead of it crossing the farm per task (the 5x movement leak).
        own = _S["own"].setdefault(ukey, set())
        own &= task_map.keys()          # drop resolved/gone tiles
        target = _S["route"].get(ukey)
        if target not in task_map or target in claimed:
            target = None
            # 1) work owned tiles first
            best = None
            for xy in own:
            ...
            if best is not None:
                ...
                # 2) claim the nearest free task and make it ours
