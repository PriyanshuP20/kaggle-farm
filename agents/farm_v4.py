...
import os
CASH_FLOOR = 120
CREW_MAX = int(os.environ.get("KAG_CREW", 12))
PLOTS_PER_UNIT = float(os.environ.get("KAG_PPU", 5))

# Herd plan: mostly geese (daily eggs, log-glut => volume-safe) plus a few
# sheep/cows — their products ride separate price curves, so diversifying
# keeps premium prices from crashing.
HERD_PLAN = (["GOOSE", "GOOSE", "GOOSE", "SHEEP", "COW", "GOOSE",
              "SHEEP", "GOOSE", "COW", "GOOSE", "SHEEP", "COW",
              "GOOSE", "GOOSE", "SHEEP", "GOOSE", "SHEEP", "COW"]
             [:int(os.environ.get("KAG_HERD", 12))])
HERD_TARGET = len(HERD_PLAN)
HERD_BUY_LAST_DAY = -12    # stop buying animals this many days before the end
...
def _agent_impl(obs, config):
    ...
    wheat_reserve = herd + 2  # shed wheat held back for tomorrow's feeding
    ...
    #    growing (planted -> crew -> cap -> more planted).
    crew_target = min(CREW_MAX, max(5, -(-planted_total // 3) + 1 + herd_total // 3))
    ...
    # 3) Wheat for the animals: keep shed above reserve (buy back is cheap).
    ...
    # 4) Land when tiles are tight, or mid/late-game with plenty of spare cash
    #    (extra quadrants host the growing ranch).
    used_tiles = planted_total + herd + len(empty_structs)
    n_extra = len(me["unlocked_quadrants"]) - 1
    want_land = (used_tiles >= unlocked_tiles - 10
                 or (herd_total >= 8 and n_extra < 3
                     and money - spend > LAND_PRICES[n_extra] * 2 + 2000))
    if (n_extra < 3 and want_land
            and money - spend > LAND_PRICES[n_extra] + 1200
            and day + 8 <= last_day and len(market) < 10):
        market.append(["BUY_LAND"])
        spend += LAND_PRICES[n_extra]

    # 5) Animals: buy the next animals in HERD_PLAN when a matching empty
    #    structure is free, up to 2/turn, once the economy has momentum.
    if (money - spend > 1200 and day < last_day + HERD_BUY_LAST_DAY
            and n_units >= 7):
        for a in HERD_PLAN[herd_total:herd_total + 2]:
            have_home = sum(1 for _, _, k in empty_structs
                            if k == ANIMAL_STRUCT[a]) > transit_by_type.get(a, 0)
            if have_home and money - spend > ANIMALS[a]["cost"] + 900 and len(market) < 10:
    ...
    if money > 900 and n_units >= 7 and day < last_day + HERD_BUY_LAST_DAY:
        have_struct = {}
        ...
        for _, _, a, _t in animals:
            ...
            have_struct[k] = have_struct.get(k, 0) + 1
        want_struct = {}
        for a in HERD_PLAN[:herd_total + 4]:
            k = ANIMAL_STRUCT[a]
    ...
    n_ranchers = 0
    if ranch_work:
        n_ranchers = min(len(units) - 1, max(1, -(-max(herd_total, structs_total, 1) // 3)))
...
def _ranch_op(ukey, pos, inv, carrying_animal, carrying_wheat, task_map, claimed,
              shed_tiles, shed, feed_set, place_map, build_map):
    ...
    # Empty-handed: fetch supplies, then run chores.
